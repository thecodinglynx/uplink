...
import { getMetrics, resetMetrics, recordPersistenceWrite, recordTick, } from '../src/instrumentation/perf';
...
describe('instrumentation metrics', () => {
    it('records tick durations', () => {
        resetMetrics();
        const now = Date.now();
        const s = createSession(makeId('m_inst'), 1, now, { defenseCount: 1 });
