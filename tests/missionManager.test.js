...
function simpleMission(id, overrides = {}) {
    return {
        id: makeId(id),
        title: id,
        type: 'standard',
        factions: [makeId('corpA')],
        difficultyBand: DifficultyBand.EASY,
        basePayoutRange: [100, 150],
        objectives: [{ id: makeId('o1'), type: 'scan', targetCount: 1 }],
        defenseTemplate: [],
        seedMode: 'FIXED',
        version: 1,
        ...overrides,
...
describe('missionManager gating', () => {
        ...
        const mission = simpleMission('m1', {
            gates: {
                ...
                requiredToolVersions: [
                    { toolId: makeId('scanner'), minVersion: 2 }, // have only v1
                ],
            },
        });
        const ctx = {
    ...
    it('accepts mission when all gates satisfied', () => {
        ...
        const mission = simpleMission('m2', {
            gates: {
                requiredFlags: ['intro_done'],
                requiredToolVersions: [{ toolId: makeId('scanner'), minVersion: 1 }],
            },
        });
        const ctx = {
            profile,
    ...
    it('purges expired missions based on hard time', () => {
        ...
        const ctx = {
            ...
            factionRepLookup: () => 0,
            now: Date.now(),
            acceptedMissions: { [mission.id]: { acceptedAt } },
        };
