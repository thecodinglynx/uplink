...
const tiers: HardwareTier[] = [
  {
    id: makeId<'HardwareTierId'>('tier1'),
    version: 1,
    concurrencySlots: 1,
    memoryCapacity: 256,
  ...
  {
    id: makeId<'HardwareTierId'>('tier2'),
    version: 1,
    concurrencySlots: 2,
    memoryCapacity: 512,
    traceResistance: 0.1,
  ...
  {
    id: makeId<'HardwareTierId'>('tier3'),
    version: 1,
    concurrencySlots: 3,
    memoryCapacity: 1024,
...
describe('upgrade manager', () => {
  ...
  it('purchases sequential tool versions improving speed', () => {
    ...
    p = v2.profile;
    expect(getToolOperationTimeModifier(tool, 2)).toBeLessThan(
      getToolOperationTimeModifier(tool, 1),
    );
  });
