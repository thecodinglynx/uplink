...
export function updateTrace(session, now) {
    ...
    for (const def of session.defenses) {
        ...
        for (const e of def.noiseEvents)
            noiseTotal += e.magnitude;
    }
...
export function addNoiseEvent(session, defenseId, magnitude, durationMs, now) {
    const def = session.defenses.find((d) => d.id === defenseId);
    if (!def)
