...
function evaluateGates(
  ...
  factionRepLookup: (factionId: FactionId) => number | undefined,
): MissionLockReason[] {
  const reasons: MissionLockReason[] = [];
  const gates: MissionGates | undefined = mission.gates;
  if (!gates) return reasons;
...
export interface MissionManagerContext {
  profile: ProfileDefinition;
...
export function getMissionAvailabilities(ctx: MissionManagerContext): MissionAvailability[] {
...
export function acceptMission(
  ctx: MissionManagerContext,
  missionId: MissionId,
): AcceptMissionResult {
  const mission = ctx.missions.find((m) => m.id === missionId);
  if (!mission) return { ok: false, reasons: [{ code: 'MISSING_FLAG', detail: 'NOT_FOUND' }] };
...
export interface AbandonMissionResult {
  ...
  expired?: boolean; // indicates mission had already expired
}

export function abandonMission(
  ctx: MissionManagerContext,
  missionId: MissionId,
): AbandonMissionResult {
...
export function purgeExpiredMissions(ctx: MissionManagerContext): MissionId[] {
  const removed: MissionId[] = [];
...
export function summarizeLockReasons(reasons: MissionLockReason[]): string {
