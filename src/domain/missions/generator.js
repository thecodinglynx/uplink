...
export function generateSimpleMissions(count) {
    const arr = [];
    for (let i = 1; i <= count; i++) {
        const band = i <= 3 ? DifficultyBand.EASY : i <= 6 ? DifficultyBand.MID : DifficultyBand.HARD;
        ...
        arr.push({
            ...
            defenseTemplate: [
                { archetype: 'FIREWALL', minTier: 1, maxTier: 2 },
                { archetype: 'IDS', minTier: 1, maxTier: 2, highNoise: i % 2 === 0 },
            ],
            reputationEffects: [],
            seedMode: 'FIXED',
            version: 1,
