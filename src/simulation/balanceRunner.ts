...
import type { DefenseTemplateSlot } from '../domain/types';

export interface BalanceSimulationOptions {
  seeds: number[];
  template: DefenseTemplateSlot[];
  // optional difficulty scaling coefficient for payout curve
  payoutScale?: number; // default 100
}

export interface BalanceMetrics {
  totalRuns: number;
  avgDefenseCount: number;
  highNoiseAdjacencyViolations: number;
  tierFrequency: Record<number, number>;
  seedSignatures: Record<string, string>; // seed -> archetype:tier|...
  avgPayout: number;
  medianPayout: number;
  stdDevPayout: number;
  traceFailureRate: number; // heuristic probability of trace failure across runs
  payoutQuantiles: { q10: number; q25: number; q75: number; q90: number };
  payoutBuckets: { low: number; mid: number; high: number }; // fraction of runs falling into each bucket
}

export function runBalanceSimulation(opts: BalanceSimulationOptions): BalanceMetrics {
  const payoutScale = opts.payoutScale ?? 100;
  let totalDefs = 0;
  let violations = 0;
  const tierFrequency: Record<number, number> = {};
  const seedSignatures: Record<string, string> = {};
  ...
  for (const seed of opts.seeds) {
    ...
    for (const d of defenses) {
      if (!tierFrequency[d.tier]) tierFrequency[d.tier] = 0;
      tierFrequency[d.tier] += 1;
      const high = !!d.highNoise;
      if (high) {
        run = lastHigh ? run + 1 : 1;
        if (run > 2) violations += 1;
    ...
    if (failureProb > 0.5) traceFailures += 1; // count as failure if surpass threshold; coarse measure
  }
  ...
  for (const p of payouts) {
    if (p <= lowCut) lowCount++;
    else if (p >= highCut) highCount++;
    else midCount++;
  }
  return {
